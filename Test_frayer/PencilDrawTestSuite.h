...
class PencilDrawTestSuite : public CxxTest::TestSuite
{
public:
	PencilDrawTestSuite()
	{
	...
	void test_EditAfterMoveLayer()
	{
		...
		{//test edit
			IIETool* pTool = m_ImgEdit.GetIEToolFromName("PencilDraw");
		...
		{//check edited layer image
			...
			//	cvSaveImage(".\\test_data\\Test_frayer\\PencilDraw\\test_EditAfterMoveLayer.bmp", layer_img);
			//	cvReleaseImage(&layer_img);
			//}

			IplImageExt correct_img_ext;
			LoadIplImageExt(".\\test_data\\Test_frayer\\PencilDraw\\test_EditAfterMoveLayer.bmp", &correct_img_ext);
