...
class EraserTestSuite : public CxxTest::TestSuite
{
public:
	EraserTestSuite()
	{
	...
	void test_EditAfterMoveLayer()
	{
		...
		{//test edit
			IIETool* pTool = m_ImgEdit.GetIEToolFromName("Eraser");
		...
		{//check edited layer image
			{
				UCvPixel pix;
				pLayer->GetPixel(100, 100, &pix);
				TS_ASSERT(pix.a == 0);
				pLayer->GetPixel(200, 100, &pix);
				TS_ASSERT(pix.a == 0);
				pLayer->GetPixel(300, 100, &pix);
				TS_ASSERT(pix.a == 0);
				pLayer->GetPixel(400, 100, &pix);
				TS_ASSERT(pix.a == 0);
			}
			...
			//	cvSaveImage(".\\test_data\\Test_frayer\\Eraser\\test_EditAfterMoveLayer.bmp", layer_img);
			//	cvReleaseImage(&layer_img);
			//}

			IplImageExt correct_img_ext;
			LoadIplImageExt(".\\test_data\\Test_frayer\\Eraser\\test_EditAfterMoveLayer.bmp", &correct_img_ext);
