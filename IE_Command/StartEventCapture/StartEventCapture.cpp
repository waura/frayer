// StartEventCapture.cpp : DLL アプリケーション用にエクスポートされる関数を定義します。
//

#include "stdafx.h"
#include "StartEventCapture.h"
#include "Cderr.h"
...
#define DISPLAY_NAME "イベントキャプチャを開始する"
...
{
	strcpy_s(dst, MAX_IE_MODULE_NAME, "StartEventCapture");
}
...
StartEventCapture::StartEventCapture()
{
  	strcpy_s(m_name, MAX_IE_MODULE_NAME, "StartEventCapture");
...
BOOL StartEventCapture::Run(ImgEdit *pEdit, void* pvoid)
{
	::OPENFILENAME ofn;
	...
	ofn.lpstrTitle = TEXT("保存先ファイルを選択してください");

	if (GetOpenFileName(&ofn) == 0) {
		return FALSE;
	}

	pEdit->StartEventCapture(file_path);
	return FALSE;
}